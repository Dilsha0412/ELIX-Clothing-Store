import React from 'react'
import { useState } from 'react';
import { FaBars } from 'react-icons/fa';
import AdminSidebar from './AdminSidebar';
import { Outlet } from 'react-router-dom';

const AdminLayout = () => {
    const [isSidebarOpen, setIsSidebarOpen] = useState(false);

    const toggleSidebar = () => {
        setIsSidebarOpen(!isSidebarOpen);
    };

    return (
        <div className='min-h-screen flex flex-col md:flex-row relative bg-neutral-50'>
            {/* Mobile Toggle Button */}
            <div className='flex md:hidden p-4 bg-black text-white border-b border-neutral-900 z-20'>
                <button onClick={toggleSidebar}>
                    <FaBars size={24} />
                </button>
                <h1 className='ml-4 text-xl font-medium tracking-wider uppercase font-semibold'>Admin Dashboard</h1>
            </div>

            {/* Overlay for mobileSidebar */}
            {isSidebarOpen && (
                <div
                    className='fixed inset-0 z-10 bg-black bg-opacity-10 md:hidden'
                    onClick={toggleSidebar}
                ></div>
            )}
            {/* Sidebar */}
            <div
                className={`bg-black border-r border-neutral-900 w-64 min-h-screen text-white absolute md:relative transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    } transition-transform duration-300 md:translate-x-0 md:static md:block z-20`}
            >
                {/* Sidebar */}
                <AdminSidebar />
            </div>

            {/* Main Content*/}
            <div className='flex-grow p-6 md:p-8 overflow-auto'>
                <Outlet />
            </div>
        </div>
    );
};

export default AdminLayout;